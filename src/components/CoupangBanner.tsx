      ...
      <div className={containerClasses[layout]}>
        {banners.map((banner) => (
          <a
            ...
            className="group relative flex flex-col justify-end overflow-hidden rounded-2xl h-52 w-full shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 border border-black/10 cursor-pointer"
          >
            {/* 배경 이미지 */}
            <div
            ...
            {/* 내부 콘텐츠 */}
            <div className="relative z-10 p-5 flex flex-col items-start gap-2 text-left">
              {/* 카테고리 뱃지 */}
              <span className={`text-[10px] font-black text-white px-2.5 py-0.5 rounded shadow-sm tracking-wider uppercase ${banner.badgeBg}`}>
                {banner.badge}
              </span>

              {/* 배너 타이틀 */}
              <h4 className="text-sm md:text-base font-black text-white leading-snug drop-shadow-sm tracking-tight mb-1 line-clamp-2">
                {banner.title}
              </h4>

              {/* 링크 이동 안내 텍스트 */}
              <span className="text-xs font-bold text-amber-400 group-hover:text-amber-300 transition-colors drop-shadow-sm flex items-center gap-1">
                {banner.linkText}
              </span>
            </div>
          </a>
